...
export async function translateWord(word: string): Promise<TranslationResponse> {
  try {
    ...
    // エラー時はモックデータを返す
    return getMockTranslation(word);
  }
}
...
function generateExamples(word: string): { en: string; ja: string }[] {
  // 実際には例文APIを呼び出すべきだが、ここではモックデータを使用
  const mockExamples = [
    { 
...
/**
 * モックの翻訳データを生成する関数（APIエラー時のフォールバック用）
 */
function getMockTranslation(word: string): TranslationResponse {
  const mockData: { [key: string]: TranslationResponse } = {
    'quick': {
      word: 'quick',
      translation: '素早い',
    ...
    'fox': {
      word: 'fox',
      translation: '狐',
      alternatives: ['キツネ', 'フォックス'],
      partOfSpeech: '名詞',
      examples: [
        { en: 'The fox ran into the forest.', ja: '狐は森の中へ走っていった。' },
        { en: 'Foxes are known for being clever.', ja: '狐は賢いことで知られている。' }
      ]
    },
    'jumps': {
      word: 'jumps',
      translation: '跳ぶ',
      alternatives: ['飛び跳ねる', 'ジャンプする', '跳躍する'],
      partOfSpeech: '動詞',
      examples: [
        { en: 'The cat jumps over the fence.', ja: '猫はフェンスを飛び越える。' },
        { en: 'He jumps at every opportunity.', ja: '彼はあらゆる機会を逃さない。' }
      ]
    }
  };
